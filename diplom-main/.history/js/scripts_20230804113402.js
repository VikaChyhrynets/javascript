...
/*
  ...
  */
  ...
  // Обработчик клика на кнопке "Next Slide"
  ...
  function updateCountdown() {
    const countDate = new Date(document.querySelector('.counter-widget').dataset.countdate).getTime();
    const now = new Date().getTime();
    const timeDifference = countDate - now;

    if (timeDifference <= 0) {
        // Если дата прошла, то останавливаем счетчик и показываем 00:00:00:00
